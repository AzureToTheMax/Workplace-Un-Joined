...
2.4
    -Changed the "A user could not be determined at this time." exit result from 1 to 0.
    This had been set this way to better represent devices where we couldn't know the results at the time.
    However, this causes issues when this script is combined with the remediation script as it triggers the remediation to run when no account is present.
    While this causes no harm, it's annoying and creates way too many logs.
